 ...
 */
class MembersJobs extends \yii\db\ActiveRecord implements IdentityInterface
{
...
//////////////////////////////////
    ...
    public function rules()
    {
        return [
			...
			[['status', 'tel','mobile', 'category'], 'integer'],
            [['name', 'auth_key'], 'string', 'max' => 50],
			[['username', 'password'], 'string', 'max' => 30],
			[['password_hash'], 'string', 'max' => 60],
		 	['email','email'],
			['username', 'unique', 'targetClass' => 'app\models\MembersJobs', 'message' => 'This username is already taken'],
			[['tel'], 'isPhoneValid'],
	...
	public function beforeSave($insert)
	{
		if (parent::beforeSave($insert)) {
			// ...custom code here...
			if ($this->isNewRecord) {
				$this->status = MembersJobs::STATUS_INACTIVE;
	...
	public static function checkAdminStatus () {
		...
		$flag = true;
		$sql = "select status FROM members WHERE username = 'admin' and status = ". MembersJobs::STATUS_INACTIVE;// and password_hash = '$passwordhash'
    ...
    /**
     * @inheritdoc
     * @return MembersJobsQuery the active query used by this AR class.
     */
    public static function find()
    {
        return new MembersJobsQuery(get_called_class());
    }
